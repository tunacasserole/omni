...
#   # def self.styles
#   #   sql = "select distinct(style_id), account_id from skus_load"
#   #   data = ActiveRecord::Base.connection.execute sql
#   #   data.each do |x|
#   #     style_id = x[0]
#   #     account_id = x[1]
#   #     update_sql = "update styles set account_id = '#{account_id}' where style_id = '#{style_id}'"
...
#  #    data = ActiveRecord::Base.connection.execute sql
#  #    data.each_with_index do |x,i|
#  #      Omni::Account.create(display: x[0], account_name: x[0], school_code: x[1]) unless (Omni::Color.where(display: x[1]).first || Omni::Color.where(short_name: x[1]).first)
#  #    end
#  #  # def self.fill_in

#  #  #   end

#  #      # sl.account_id = Omni::Account.where()
