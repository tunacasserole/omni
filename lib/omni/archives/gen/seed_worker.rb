...
class Omni::Gen::SeedWorker <  Omni::Gen::Worker

  @@folder_name = File.join(Rails.root, 'vendor','gems','omni','db','meta')
  ...
  def self.file_path
    File.join(Rails.root.to_s, 'vendor', 'gems', 'omni', 'db', 'seed')
  end

  def self.error_file_path
    File.join(Rails.root.to_s, 'vendor', 'gems', 'omni', 'db', 'error')
  end
...
#  end # def self.destroy
          ...
          if key_name != 'main!A1' and key_name != ' ' and key_name != '' and !@@skip_these.include? key_name
            ...
            if key_name.end_with? '_id' and !key_name.end_with? 'user_id'
              ...
              if !parent
                ...
                parent_row = ('Omni::' + parent).constantize.where(:display => key_value).first
                if !parent_row
  ...
  def self.table_contents(mark_model)
    ...
    model = ('Omni::Mark' + mark_model +'Dev').constantize
