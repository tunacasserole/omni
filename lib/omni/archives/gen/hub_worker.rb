class Omni::Gen::HubWorker < Omni::Gen::Worker
  ...
  def self.hub_contents(hub, gem_name)
    ...
    #model_headers = File.join(Rails.root,'vendor/gems/omni/db/meta/model_headers.xlsx')
    ...
    sections = Omni::Meta::Base.excel_to_hash meta_folder, meta_file, 'sections'
    tiles = Omni::Meta::Base.excel_to_hash meta_folder, meta_file, 'tiles'    
    ...
    sections.each do |section|
      ...
      tiles.each do |tile|
       ...
       ### SAMPLE {title: 'Release Title 1 Pick Tickets',  colspan: 2, rowspan: 1, cls: 'release_pick_tickets purple1', badge: '3', target: {xtype: 'omni-pick_tickets-Explorer', title: 'Release Title 2 Pick Tickets',  defaultSearch: {with: {state: {equal_to: 'new'}}}, contextMenuConfig: {xtype: 'omni-pick_tickets-ExplorerContextMenuRelease'}, store: Ext.create('Omni.store.PickTicket', {storeId: 'ReleasePickTicketStore'}), allowNew: false}},        #puts "tiles" + tile['color']
        ...
        #xtype = "xtype: 'omni-#{table_name}-Explorer'"
        xtype = "xtype: 'omni-#{tile['model_name'].tableize}-Explorer'"
        ...
        if tile['event']
          ...
          contextMenuConfig = "contextMenuConfig: {xtype: 'omni-#{table_name}-ExplorerContextMenu#{tile['event'].titleize}'}"
