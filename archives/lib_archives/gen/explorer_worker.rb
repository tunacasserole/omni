...
class Omni::Gen::ExplorerWorker < Buildit::Gen::Scaffold::Worker
...
Ext.define('<%= model.namespace %>.view.<%= model.model_name.tableize %>.<%= explorer.explorer_name %>ContextMenuEvent', {
  ...
  clickEvent: function(btn, e, eOpts){
    Omni.logic.<%= model.table_name %>.ExplorerProcessSelectedItems.click(btn, 'event');
  },
        ...
        function(btn){
          if(btn == 'yes'){
            Ext.each(selectedItems, function(item){
              Omni.service.<%= model.model_name %>.fireEvent(
                {
                  name: event_name, 
...
end # class Omni::Gen::ExplorerWorker