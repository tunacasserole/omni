class Omni::Import::ExcelCsv::Style < Omni::Import::Base
  ...
  def self.import(import)
    ...
    @data.each_with_index do |row,i|
      ...
      x.product_id = Omni::Product.where(:display => row.product_id).first      
      x.brand = row.brand.upcase if row.brand
      x.product_type_id = Omni::ProductType.where(:display => row.product_type_id).first            
      ...
      if generic_style
        x.generic_style = generic_style.style_id 
      else
        puts "--GENERIC STYLE is not valid."
      end

      site=Omni::Site.where(:display=>row.site_id).first || Omni::Site.create(:display => row.site_id)
      if site
      ...
      supplier=Omni::Supplier.where(:display=>row.supplier_id).first || Omni::Supplier.create(:display => row.supplier_id)
      if supplier
      ...
      size_group=Omni::SizeGroup.where(:display=>row.size_group_id).first || Omni::SizeGroup.create(:display => row.size_group_id)
      if size_group
