class Omni::Import::GenericData < Omni::Import::Base
    ...
    @@core_data.each do |row|
      ...
      row.keys.reject {|k| !row[k] or row[k] == ' ' or row[k] == 'main!A1'}.each do |a_name_original|
        ...
        if a_name.end_with? '_id'
          ...
          if !parent
            exceptions << "Sorry, couldn't locate parent model for #{a_name}\n"
            next
          end
  ...
  def self.get_parent(attribute_name)
    puts "\n\n\n ... getting parent ... \n\n\n"
    return attribute_name.chop.chop.chop.classify
    # puts "the parent_modelnt of #{attribute_name} is parent_model"
  end
end
