class Omni::Sync::Mark < Omni::Import::Base
  ...
  def self.load
    ...
    @locations = Omni::Location.outlet_hash
    @skus = Omni::Sku.stock_hash 
  ...
  def self.order_hashes
    ...
    @order_to_outlet = {}
    @order_to_date = {}
    Omni::MarkOrder.where('order_nbr >= ?',self.last_order_of_2010).find_each do |o|
      @order_to_date[o.order_nbr] = o.date_putin
  ...
  def self.on_hand
    ...
    ActiveRecord::Base.transaction do
      Omni::MarkInventory.where('qoh > 0 and stock_nbr > 0 and size is not null').find_each do |x|
        ...
        if !location_id
          @no_location_count += 1
          @no_locations << x.outlet_nbr.to_s if x.outlet_nbr
          next
        end
  ...
  def self.wip
    load
    # puts "# of wip rows:  #{Omni::MarkWip.where('cut_wip > 0 or plant_wip > 0 or cont_wip > 0').count.to_s}"
    ActiveRecord::Base.transaction do    
      Omni::MarkWip.where('cut_wip > 0 or plant_wip > 0 or cont_wip > 0').find_each do |x|
        ...
        @created_count += 1
        # puts "==== created count: #{@created_count}"
      end
    end
  ...
  def self.sold_simple
    load
    order_to_date = Omni::MarkOrder.order_to_date_hash  
    ...
    Omni::MarkOrderLine.where('order_nbr >= ?', Omni::MarkOrder.last_order_of_2010).find_each do |line|
      # puts "stock nbr: #{line.stock_nbr.to_s}, size: #{line.size}"
      ...
      @created_count += 1 
      puts "#{Time.now.strftime("%H:%M:%S").yellow}: processing row: #{@created_count.to_s}" if @created_count.to_s.end_with? '0000' 
      break if @created_count > 25000    
    end
