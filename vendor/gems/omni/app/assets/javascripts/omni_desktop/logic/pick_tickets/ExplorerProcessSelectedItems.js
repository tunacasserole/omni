 ...
 */
Ext.define('Omni.logic.pick_tickets.ExplorerProcessSelectedItems', {
  statics: {
    click:function(btn, event_name){
      ...
      Ext.Msg.confirm(
        ...
        'You are about to process the selected Pick Ticket(s), are you sure?',
        function(btn){
          if(btn == 'yes'){
            Ext.each(selectedItems, function(item){
              Omni.service.PickTicket.fireEvent(
                {
                  name: event_name, 
                  id: item.get('pick_ticket_id')
                },
                function(response, e){
