...
class Omni::Purchase::Helpers < Omni::Purchase
  ...
  def self.print(po)
    ...
    Prawn::Document.generate(file_name, :template => po_logo) do
      ...
      draw_text "TO:  #{po.supplier.display}", :size => 9, :at => [8, 640]
      draw_text "L-A-2/B BLOCK 21", :size => 9, :at => [16, 630]
      draw_text ",FE PAKISTAN", :size => 9, :at => [16, 610]         
                          
      draw_text "DATE OF ORDER:  #{po.order_date}", :size => 9, :at => [210, 640]          
      draw_text "SHIP DATE:  #{po.order_date}", :size => 9, :at => [210, 630]
      ...
      po.purchase_details.each_with_index do |pd,i|
