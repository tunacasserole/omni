class CreateOmniPieces < ActiveRecord::Migration
  def change
  	#ActiveRecord::Base.establish_connection(Buildit::Util::Data::Connection.for('BUILDIT'))
    @connection = ActiveRecord::Base.connection
  	unless ActiveRecord::Base.connection.tables.include?('pieces')
      create_table(:pieces, :id => false) do |t|
        t.column   :piece_id,                        :string,            :null  =>  false,   :limit   => 32
        t.column   :project_id,                      :string,            :null  =>  true,    :limit   => 32
        t.column   :piece_nbr,                       :string,            :null  =>  true,    :limit   => 100
        t.column   :piece_type,                      :string,            :null  =>  true,    :limit   => 100
        ...
        t.column   :importance,                      :string,            :null  =>  true,    :limit   => 100
